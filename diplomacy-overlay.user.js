...
// @downloadUrl  https://github.com/thmsndk/screeps-browser-ext/raw/master/diplomacy-overlay.user.js
// ==/UserScript==
...
function generateColor(hue, saturation, lightness) {
  for (let i = 0; i < 100; i++) {
    let color = hslToRGB(
      ...
      0.1 * Math.round((0.2 * Math.random()) / 0.1) + saturation,
      0.1 * Math.round((0.2 * Math.random()) / 0.1) + lightness
    );
    if (!_.some(colorMap, (existing) => _.eq(color))) return color;
  }
}
...
function hslToRGB(a, b, c) {
  ...
  var d,
    e,
    f,
    g = (1 - Math.abs(2 * c - 1)) * b,
    h = a / 60,
    i = g * (1 - Math.abs((h % 2) - 1));
...
function generateAndSetColor(userid, userName) {
  ...
  switch (diplomacyScore) {
    case -1:
      color = generateColor(-15, 0.8, 0.4);
      break;
    case 1:
      color = generateColor(210, 0.8, 0.5);
      break;

    default:
    case 0:
      color = generateColor(40, 0.8, 0.35);
      break;
  }
...
function prepareRoomObjects(scope, element, roomHandle, mapScale) {
  ...
  element[0].listenerEvent = ScreepsAdapter.Socket.bindEventToScope(
    scope,
    `roomMap2:${roomHandle}`,
    function (objects) {
...
function replaceUnitsToggle() {
  ...
  if ($("#diplomacy-units").length) return;
